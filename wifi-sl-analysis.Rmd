...
library(tidyverse) # to read, maniuplate and graph data
...
d_pass_long %>% ggplot(aes(x = fct_reorder(scenario, as.numeric(is_acceptable), .fun = "mean", .desc = TRUE),
  ...
  coord_flip()
ggsave("acceptbyscen.png", width = 5, height = 2.5, units = "in", dpi = 300)
```
...
# boxplot of privcy dimension ratings by each scenario
...
ggsave("dimbyscenbox.png", width = 7.3, height = 9, units = "in", dpi = 300)
...
d_pass_long %>% ggplot(aes(x = reorder_within(scenario, rating, dimension), y = rating, 
  ...
  xlab("") +
  ylab("Likert rating (0-5)")
ggsave("scenbydimbox.png", width = 7.3, height = 9, units = "in", dpi = 300)
...
demo_df <- d %>% dplyr::select(ResponseId, age:education) %>% # save only demographic data
                anti_join(failed_attcheck, by = "ResponseId") # remove failed attention check

demo_df <- demo_df[rep(c(TRUE, FALSE, FALSE), 287), ] # remove duplicates
...
# REL TO UNIMELB
  ...
  counts <- demo_df %>% count(rel_to_unimelb)
...
ggsave("demo-relunimelb.png", width = 3.5, height = 3.5, units = "in", dpi = 300)
```
...
### get datset with proportion of acceptance for each participant (0, .33, .66, 1)
...
lm(mean ~ education, data = meanaccept) %>% aov %>% summary() #eductaional attainment
...
fillcol <- "lightblue"
ypos <- c(0.925) #ypos for ggsignif
...
gender_acc <- meanaccept %>% drop_na(gender) %>% ggplot(aes(x = gender, y = mean)) + 
  ...
  theme(text = element_text(family = "Myriad Pro", size = 8)) 
ggsave("demoacc-gen.png", width = 2.5, height = 2.5, units = "in", dpi = 300)
...
edu_acc <- meanaccept %>% drop_na(education) %>%
  ...
  ggsignif::stat_signif(comparisons = list(c(1, 9)),
                        ...
                        y_position = ypos,
                        textsize = geom.text.size, vjust = 0)
ggsave("demoacc-edu.png", width = 2.5, height = 2.5, units = "in", dpi = 300)
...
age_acc <- meanaccept %>% drop_na(age_group) %>% ggplot(aes(x = age_group, y = mean)) + 
  ...
  ggsignif::stat_signif(comparisons = list(c(1, 5)),
                        ...
                        y_position = ypos,
                        textsize = geom.text.size, vjust = 0)
ggsave("demoacc-age.png", width = 2.5, height = 2.5, units = "in", dpi = 300)
...
relunimelb_acc <- meanaccept %>% drop_na(rel_to_unimelb) %>%
  ...
  ggplot(aes(x = rel_to_unimelb, y = mean)) + 
    ...
    ggsignif::stat_signif(comparisons = list(c(1, 6)),
                        ...
                        y_position = ypos,
                        textsize = geom.text.size, vjust = 0)
ggsave("demoacc-reltounimelb.png", width = 2.5, height = 2.5, units = "in", dpi = 300)
...
ggsave("demoBIG.png", width = 5, height = 7.5, units = "in", dpi = 300)
...
main_effects <- paste0(colnames(d_pass)[5:15], collapse = " + ")
...
postINT
ggsave("posteriorsINT.png", width = 3.5, height = 3.5, units = "in", dpi = 300)
...
ggsave("randposteriorsscen.png", width = 5, height = 5, units = "in", dpi = 300)
...
randpart <- mod_noint %>%
  ...
  theme(text = element_text(family = "Myriad Pro", size = 8),
        axis.text.y = element_blank()) +
  geom_vline(xintercept = 0, linetype = "dashed") 
ggsave("randposteriorspart.png", width = 5, height = 9, units = "in", dpi = 300)
  
# JOIN GRAPHs together 
cowplot::plot_grid(randscen, randpart, ncol = 2, rel_widths = c(3, 2), labels = 'AUTO', hjust = 0)
ggsave("randCOMB.png", width = 7, height = 5, units = "in", dpi = 300)
...
ggsave("modCOMB.png", width = 7, height = 3.5, units = "in", dpi = 300)
showtext_auto(FALSE)
...
mod1 <- glmer(formula_noint_allrand, data = training, family = binomial(link = "logit"), 
              control= glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

# get and analyse predictions on test data

test$mod1_prob <- predict(mod1, test, re.form = NA, type = "response")
...
```{r pred-mod2}
# get formula
formula_noint <- paste0("is_acceptable ~ ", main_effects)

# fit model to training data
mod2 <- glm(formula_noint, data = training, family = binomial(link = "logit"))
...
# Train the model
mod3 <- train(form = as.formula(formula_noint),
                           data = training,
                           method = "glmnet",
                           family = "binomial",
